...
Цвета объектов карты задаются в виде индексов в 32-цветной палитре. Данный диалог позволяет настроить RGB-CMYK значения цветов в этой палитре. Для изменения цвета следует выбрать соответствующую ячейку отображающую цвет с определенным индексом. При этом в полях палитры RGB (красный, зелёный, синий) будет выведено разложение текущего цвета, а окно цвета закрасится соответствующим цветом. Значения разложения можно изменить. Окно цвета меняет окраску, и отображает цвет с заданными RGB-значениями. При изменении цвета в ячейке палитры, меняется цвет у всех объектов карты, графика которых привязана к данной ячейке.
...
![alt-текст](../_assets/but_layers_layer_add.png) — добавление нового слоя, указанного в окне «Легенда», в карту;
